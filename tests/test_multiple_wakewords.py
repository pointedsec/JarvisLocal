...
class TestMultipleWakewordsConfig(unittest.TestCase):
    ...
    @patch("voice_assistant.config_manager.os.path.exists", return_value=False)
    @patch("argparse.ArgumentParser.parse_args")
    def test_comma_separated_wakewords(
    ...
    @patch("voice_assistant.config_manager.os.path.exists", return_value=False)
    @patch("argparse.ArgumentParser.parse_args")
    def test_single_wakeword_backward_compat(
        self, mock_parse_args, mock_exists, mock_sanitize
    ):
    ...
    @patch("voice_assistant.config_manager.os.path.exists", return_value=False)
    @patch("argparse.ArgumentParser.parse_args")
    def test_wakewords_whitespace_handling(
        self, mock_parse_args, mock_exists, mock_sanitize
    ):
        ...
        mock_namespace = argparse.Namespace(
            ...
            **{
                **DEFAULT_SETTINGS,
                "wakewords": "  hey jarvis ,  jarvis  , oye jarvis  ",
            },
        )
        mock_parse_args.return_value = mock_namespace
    ...
    @patch("voice_assistant.config_manager.os.path.exists", return_value=False)
    @patch("argparse.ArgumentParser.parse_args")
    def test_empty_wakewords_entries_filtered(
        self, mock_parse_args, mock_exists, mock_sanitize
    ):
...
class TestWakewordTrimming(unittest.TestCase):
    ...
    def _create_mock_assistant(self, wakewords_list):
        ...
        assistant.args = args
        assistant._trim_wakeword = VoiceAssistant._trim_wakeword.__get__(
